...
- The user only wants a quality review of the spec document without making changes → use `pragmatic-arch-spec-validate`

**Key distinction from `pragmatic-arch-spec-check`:** `pragmatic-arch-spec-check` is read-only diagnosis. This skill is the write operation triggered by its findings (Option B) or by any new architectural decision that must be formally recorded.
...
- Is this capturing an **intentional deviation** identified by `pragmatic-arch-spec-check`?
