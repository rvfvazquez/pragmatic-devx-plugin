...
The definition of done is explicit: the implementation is complete when `spec.check` returns PASS.
...
Use this skill after a spec has been validated and approved. Do not use it as a shortcut to skip `spec.create` or `spec.validate`.
...
- The spec is in Draft status or has open TODOs → resolve with `spec.validate` + `spec.update` first
- The user only wants to verify existing code → use `spec.check`
- The user wants to update the spec document → use `spec.update`
...
- If `Status: Draft` → **STOP**. Assert: "This spec is in **Draft** status. Running `spec.build` on an incomplete spec will produce an implementation that does not match the final decisions. Run `spec.validate` and resolve FAIL items first, then use `spec.update` to set the status to Review or Approved."
- If the spec has open `[TODO: ...]` items in technology decisions or acceptance criteria → **STOP**. Assert: "This spec has **N unresolved TODO item(s)** in critical sections. Implement from an incomplete spec and the result will not conform to `spec.check`. Resolve them with `spec.update` first."
...
Check `.claude/rules/` for any rule files that apply to this feature's domain or module (e.g., `*-arch.md` files generated by `arch.spec.validate`).
...
> **Next step:** Run `spec.check` against `docs/specs/<feature-slug>.md` to verify that the implementation conforms to spec. The feature is not complete until `spec.check` returns PASS.
