...
```
[project.constitution] → project.constitution.update
         ↓ read by
spec.create · arch.spec.create · spec.build · spec.check
```
...
**Do not use when:**
- A constitution already exists and the user wants to modify it → use `project.constitution.update`
- The rule belongs to a specific module → use `arch.spec.create` or `arch.spec.update`
...
If it **exists**:
> "A project constitution already exists at `docs/constitution.md`. This skill will **replace** it. Use `project.constitution.update` to make targeted changes instead.
>
...
> **These rules are now active.** Claude Code loads `.claude/rules/00-project-constitution.md` automatically at the start of every session. All future `spec.create`, `arch.spec.create`, and `spec.build` runs will use these as context.
>
> **Next step:** run `spec.create` for your first feature spec or `arch.spec.create` for the first module — the constitution will be read automatically.
